...
from functions_for_texts import *
import pandas as pd
import os
...
# Read stopwords
with open("C:/Users/Dorothea/PycharmProjects/Diploma_Thesis/Dataset/stopwords-el.txt", 'r',
          encoding='utf-8', errors='ignore') as f:
    stop_words = f.read().split('\n')

stops = []
for s in stop_words:
    stops.append(strip_accents_and_lowercase(s))
...
df = pd.read_excel("C:/Users/Dorothea/PycharmProjects/Diploma_Thesis/Dataset/Articles_preprocessed.xlsx")
doc_text = df.iloc[:, 2].tolist()
docs = []
for i in range(len(doc_text)):
    docs.append(strip_accents_and_lowercase(doc_text[i]))

'''
doc_text = df.iloc[:, 4].tolist()
docs = []
for i in range(len(doc_text)):
    if doc_text[i] == "None":
        doc = df.iloc[i, 2]
        ...
        doc = doc_text[i]
    docs.append(strip_accents_and_lowercase(doc))
'''
# df = pd.read_excel("../la.xlsx")
# docs = df.loc[df['cluster'] == 6].iloc[0:, 1].tolist()
...
file = open("./article_ids.txt", "w")
file.write("train\n")
for i in range(trSize):
    print(idx_permute[i])
    file.write(str(df.iloc[idx_permute[i], 1]) + "," + str(idx_permute[i]) + "\n")
print("---------------------------------------")
file.write("--\n")
file.write("test\n")
for i in range(trSize, trSize+41):
    print(idx_permute[i])
    file.write(str(df.iloc[idx_permute[i], 1]) + "," + str(idx_permute[i]) + "\n")
print("---------------------------------------")
file.write("--\n")
file.write("valid\n")
for i in range(trSize+41, len(docs)):
    print(idx_permute[i])
    file.write(str(df.iloc[idx_permute[i], 1]) + "," + str(idx_permute[i]) + "\n")
file.close()
